...
//     $(this).attr('value', $('#datetimepicker1').val());
// }).data('datepicker');
...
//         $("#datetimepicker2").val(dateText);
//       }
// }).on('changeDate', function (ev) {
//     $(this).attr('value', $('#datetimepicker2').val());
//     checkout.hide();
// }).data('datepicker');
//======================
...
var HANDLE_PRODUCT_ADD = {
    ...
    errors: {
        ...
        "qty_field_required": "Please enter valid offered price.",
        "greater_qty_field_required":"Offered price can not be greater then or equal to actual price"
    },
    ...
    __fetch_quantity_product: function (id, product_id) {
        ...
        $.ajax({
            ...
            url: 'ajax-request/store-offer/get-product-qyt',
...
$('body').on('click', '#show_preview_data', function () {
    ...
    $('.offered-price').each(function () {
        ...
        if(parseFloat(offer_price) >= parseFloat(actual_price))
        {
...
$("body").on('click','#step_three_add_date',function(){
  ...
  let currentDate=moment().format('YYYY-MM-DD');
    ...
    if($.trim(endDate)< currentDate) {

        swal("Offer end date can not less than current date.");
        return false;
    }
