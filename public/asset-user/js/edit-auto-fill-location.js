          ...
          navigator.geolocation.getCurrentPosition(function (position) {
            ...
            $.ajax({
              ...
              success: function (location) {
                $('#country-edit').val(location.country_name);
                $('#administrative_area_level_1-edit').val(location.state);
                $('#locality-edit').val(location.city);
                $("#postal_code-edit").val(location.postal);
                $('#ip-edit').val(location.IPv4);
              }
            });
            //end code
  ...
  function codeLatLng(lat, lng) {
    ...
    geocoder.geocode({ 'latLng': latlng }, function (results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
        $("#location-edit").val(results[0].formatted_address);
      } else {
