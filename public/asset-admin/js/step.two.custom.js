            ...
            success: function (response) {
                    ...
                    if(parseInt(response.code) == 200) {
                        ...
                        if(response.data && response.data.service_days.length > 0) {
                            let html = '<tr class="timeslot_heade">'+
                            '<td>Day </td>'+
                            '<td> Select Day</td>'+
                            '<td colspan="2">'+
...
$('body').on('click', '.add_button_slot', function () {

    var ID = $(this).attr('data-attr-service-id');
    var tab = $(this).attr('data-attr-tab');
    $("#hidden_service_id").val(ID);
    $("#myModalrt-time").modal('show');
    $("#hidden_tab_value").val(tab);
});
...
$(document).ready(function(){
    ...
    $('.accordian-block .ac-title').eq(0).trigger('click');
