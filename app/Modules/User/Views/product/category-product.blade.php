...
@include('User::includes.navbar')
         ...
         <!--header close-->
      @section('content')
   <form action="{{route('users.category.product',['id'=>encrypt($category_id)])}}" method="get" id="filterFormId">
      @yield('nav-bar')        
       <!--section list filter-->
            ...
            <div class="custom_inner_conatiner">
               <div class="flex-row">
                  <div class="flex-col-sm-3">
                     ...
                     @if(isset($query) && !empty($query))<a href="{{route('users.category.product',['id'=>encrypt($category_id)])}}"><span class="clear_filter">Clear All</span></a>@endif
                  ...
                  <div class="flex-col-sm-9">
                     <div class="flex-row align-items-center">
                        <div class="flex-col-sm-6 flex-col-xs-6">
                        <h2 class="title-heading m-t-b-30">@if(isset($category_id) && !empty($category_id)){{$productCategory['data'][0]['category_name'] ?? ''}}@endif</h2>
                        </div>
                   ...
                   @foreach ($productCategory['data'] as $item)
                     ...
                     <div class="scroller-status">
                      ...
                      <!--scroller end pagination -->
                  </div>
                      <!--pagination -->
                              <div class="pagination">
                                  <a href="@if($productCategory['nextPage']) {{route('users.product.category').'?page='.$productCategory['nextPage']}} @else javascript:void(0); @endif" class="next">Next</a>
                               </div>
