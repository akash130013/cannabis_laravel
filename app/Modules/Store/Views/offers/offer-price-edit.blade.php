                     ...
                     <span class="progress_bar">
                        ...
                        <div class="inner_container">
                            ...
                            @if($productStock)
                              ...
                              <div class="form_wrapper">
                                 <div class="form-group quantity-input-field">
                                    ...
                                    <input type="hidden" name="price[]" onkeypress="validate(event)" placeholder="Last actual price set is :{{$val->actual_price}}" value="{{$val->actual_price}}" class="form-control pl-230 add-product" >
                                       <p name="price[]" disabled onkeypress="validate(event)" value="{{$val->actual_price}}" placeholder="Last actual price set is :" class="form-control pl-230 ui-autocomplete-input" >{{$val->actual_price}}</p>
                                      
                                    </div>
                                 </div>
                                 <div class="form-group">
                                    <div class="text-field-wrapper">
                                          <input type="number" name="offerprice[]" onkeypress="validate(event)" placeholder="Offered price:{{$val->offered_price}}" class="offr-input offered-price"  actual_price="{{$val->actual_price}}">
                                    </div>
                                 </div>
                              </div>
                              @endforeach
                           ...
                           <div class="next-btn">
                              ...
                              <button type="button" class="primary_btn green-fill nextbtn-2 m-l-20 btn-effect" id="show_preview_data">Next</button>
                           </div>
                        </div>
                </div>
</form>
